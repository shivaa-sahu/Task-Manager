    ...
    const fetchCompletedTasks = async () => {
      try {
        const res = await fetch(
          `${API}/tasks/completed?page=${page}&limit=${limit}`,
          authHeaders(storedToken)
        );
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">
          <span className="text-green-600">{username}'s</span> Completed Tasks
        </h1>
        <div
            ...
            {tasks.map((task) => (
              ...
              >
                <div>
                  ...
                  <div className="text-sm font-semibold text-green-700">Status: {task.status}</div>
                </div>
                <button
